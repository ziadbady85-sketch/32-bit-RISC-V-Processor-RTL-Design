...
ALU_Control MA (.ALU_OP(ALU_OP),.funct3(INST_OUT[14:12]),.funct7(INST_OUT[31:25]),.OP(OP)) ;
MUX #(.w(w)) MB (.A(out_2),.B(out_1),.sel(A_2_sel),.out(out_adder)) ;
MUX #(.w(w)) MC (.A(Immediate),.B(re_data_2),.sel(ALU_scr),.out(out_reg_file)) ;
MUX #(.w(w)) MD (.A(Re_data),.B(ALU_OUT),.sel(mem_2_reg),.out(Data_Out)) ;
